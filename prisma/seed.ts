...
async function main() {
  ...
  if (existingCategoryCount === 0) {
    ...
    console.log(`ℹ Categories already exist (${existingCategoryCount} found)`);
  }

  // 3. Create Sample Products only if no products exist
  ...
  if (existingProductCount === 0) {
    const productsData = [
      {
        ...
        categoryId: categories['keychains']?.id || Object.values(categories)[0]?.id,
        price: 299,
        compareAtPrice: 399,
      ...
      },
      {
        ...
        categoryId: categories['figures']?.id || Object.values(categories)[0]?.id,
        price: 699,
        compareAtPrice: 899,
        images: JSON.stringify([
      ...
      },
      {
        ...
        categoryId: categories['decor']?.id || Object.values(categories)[0]?.id,
        price: 549,
        compareAtPrice: 699,
        images: JSON.stringify([
      ...
      },
      {
        ...
        categoryId: categories['desk-utility']?.id || Object.values(categories)[0]?.id,
        price: 349,
        compareAtPrice: 449,
        images: JSON.stringify([
      ...
      },
      {
        ...
        categoryId: categories['personalised']?.id || Object.values(categories)[0]?.id,
        price: 799,
        compareAtPrice: 999,
        images: JSON.stringify([
          'https://images.unsplash.com/photo-1615655406736-b37c4fabf923?auto=format&fit=crop&w=800&q=80',
    ...
    console.log('✔ Products seeded (initial setup)');
  } else {
    console.log(`ℹ Products already exist (${existingProductCount} found), preserving existing products`);
  }

  // 4. Create Initial Coupons only if none exist
  ...
  if (existingCouponCount === 0) {
    ...
    console.log(`ℹ Coupons already exist (${existingCouponCount} found)`);
  }
